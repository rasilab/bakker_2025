...
- [Generate the recorder-position comparison](./scripts/02_generate_figure2c_recorder_positions.R) from the repository root. This preserves the recent WT/MUT comparison for TadA–λN and TadA8.20 at 250 nM, 37 °C, and two hours.
- Each of the 32 points is the equally weighted mean of 15 construct-level editing fractions. Each error bar is the sample standard deviation divided by the square root of 15. Each construct fraction is the edited UMI count at that adenosine divided by total UMIs for the construct.
- The 15 constructs comprise four recorder layouts times four stem-mutation windows, minus the excluded spacer5_0/window 1_3 pair. The same exclusion applies to WT and MUT for both enzymes. These are construct-level measurements from one reaction per enzyme, not 15 independent biological replicates or 15 sequencing reads.
- WT/MUT comparisons use 15 matched pairs per enzyme and position, with two-sided paired t-tests and Benjamini–Hochberg correction across the 16 comparisons.
- The [summary table](./tables/fig2c_recorder_positions.csv) records the mean, standard error, sample size, and statistics for each point. Figure artifacts: [PNG](https://github.com/rasilab/bakker_2025/blob/master/analysis/boxb_in_vitro_sequencing/figures/fig2c_recorder_positions.png?raw=1) and [PDF](https://github.com/rasilab/bakker_2025/blob/master/analysis/boxb_in_vitro_sequencing/figures/fig2c_recorder_positions.pdf?raw=1).
- This script still reads [shared annotated recorder counts](./tables/target_data_processed.csv), generated by [shared preparation](./scripts/01_prepare_data.R), and the committed [stem definitions](./tables/boxb_wt_mut_stems.csv). Shared preparation runs from its script directory; its large generated tables are not committed.
- Retired the old single-enzyme eight-adenosine plot, its dedicated generator and summary table, and the obsolete position/context composite. Removed their plotting blocks from the legacy R and notebook sources while retaining randomized-context heatmaps and shared context calculations.

![Figure 2C recorder-position comparison](https://github.com/rasilab/bakker_2025/blob/master/analysis/boxb_in_vitro_sequencing/figures/fig2c_recorder_positions.png?raw=1)
...
- Figure 2C, inside the analysis container after shared preparation:

  ```bash
  Rscript analysis/boxb_in_vitro_sequencing/scripts/02_generate_figure2c_recorder_positions.R
  ```
...
- Regenerated Figure 2C after making its paths relative to the repository root: all 32 summary rows are unchanged and report n = 15; the PNG and PDF rendered at 144 DPI are pixel-identical to the recent WT/MUT plot. The legacy R sources parse after removal of the obsolete plotting blocks.
